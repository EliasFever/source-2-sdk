...
public sealed class ParticleDecal : Particle.BaseListener
{
	private readonly ParticleDecalRenderer _renderer;
	private readonly DecalDefinition _selectedDecal;
	private readonly List<ActiveDecal> _activeDecals = new();
	...
	public override void OnDisabled( Particle p )
	{
		if ( !_spawnedAny )
		{
			if ( TryGetCollisionLock( p, _renderer.ForceSpawnOnDisableCollision, out var lockData )
		...
		if ( _renderer.PersistDecalAfterParticleDeath )
		{
			...
			for ( int i = 0; i < _activeDecals.Count; i++ )
			{
				...
				_renderer.AdoptPersistentDecal( so, life, active.SpawnIndex + 1 );
			}
	...
	{
		if ( !_renderer.BlockSpawnIfParticleWouldDieOnCollision )
			return false;
	...
	private void ApplyTransform( DecalSceneObject so, in Particle p, in ActiveDecal active )
	{
		...
		so.Transform = tx;
		_lastLockedPosition = pos;
		_lastLockedNormal = normal;
		_lastLockedRotation = tx.Rotation;
		_lastLockedVolume = tx.Scale;
	}

	private void ApplyVisuals( DecalSceneObject so, in Particle p, in ActiveDecal active )
	{
	...
	private uint GetSortBaseId( in CollisionLock lockData )
	{
		...
		if ( _renderer.SortPerHitObject && lockData.HitObject is not null && lockData.HitObject.IsValid() )
	...
	public override void OnUpdate( Particle p, float dt )
	{
		...
		for ( int i = _activeDecals.Count - 1; i >= 0; i-- )
		{
			...
			if ( _renderer.ShouldCullByDistance( so.Transform.Position, persistent: false ) )
				DeleteActiveDecalAt( i );
		}
		...
		bool hasAnyVisible = _activeDecals.Count > 0;
		_renderer.ReportDebugState(
			in p,
			hasCollision: p.HitTime > 0,
			passedFilters: _lastPassedFilters,
			locked: hasAnyVisible || hasCollision,
			visible: hasAnyVisible,
			status: _lastStatus,
			hitObjectName: _lastHitObjectName,
