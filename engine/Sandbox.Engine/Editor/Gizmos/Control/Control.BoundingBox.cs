	...
	public sealed partial class GizmoControls
	{
		...
		public bool BoundingBox( string name, BBox value, out BBox outValue, out bool outPressed, out Vector3 outResizeAxis )
		{
			...
			using ( Scope( name ) )
			{
				...
				if ( ArrowPoint( "Forward", Vector3.Forward, halfSize.x, Settings.GlobalSpace ? Gizmo.Colors.Forward : Gizmo.Colors.Local.Forward, out var fd, ref pressed ) )
					(resizeDist, resizeAxis) = (fd, Vector3.Forward);

				if ( ArrowPoint( "Backward", Vector3.Backward, halfSize.x, Settings.GlobalSpace ? Gizmo.Colors.Forward : Gizmo.Colors.Local.Forward, out var bd, ref pressed ) )
					(resizeDist, resizeAxis) = (bd, Vector3.Backward);

				if ( ArrowPoint( "Up", Vector3.Up, halfSize.z, Settings.GlobalSpace ? Gizmo.Colors.Up : Gizmo.Colors.Local.Up, out var ud, ref pressed ) )
					(resizeDist, resizeAxis) = (ud, Vector3.Up);

				if ( ArrowPoint( "Down", Vector3.Down, halfSize.z, Settings.GlobalSpace ? Gizmo.Colors.Up : Gizmo.Colors.Local.Up, out var dd, ref pressed ) )
					(resizeDist, resizeAxis) = (dd, Vector3.Down);

				if ( ArrowPoint( "Left", Vector3.Left, halfSize.y, Settings.GlobalSpace ? Gizmo.Colors.Left : Gizmo.Colors.Local.Left, out var ld, ref pressed ) )
					(resizeDist, resizeAxis) = (ld, Vector3.Left);

				if ( ArrowPoint( "Right", Vector3.Right, halfSize.y, Settings.GlobalSpace ? Gizmo.Colors.Left : Gizmo.Colors.Local.Left, out var rd, ref pressed ) )
